...
function cvt_register_block() {
    // Register the block script
    wp_register_script(
        'cvt-block-editor',
        CVT_PLUGIN_URL . 'src/block.js',
        array('wp-blocks', 'wp-element', 'wp-editor'),
        CVT_PLUGIN_VERSION
    );

    // Register the block
    register_block_type('christine-valmy/chat-interface', array(
        'editor_script' => 'cvt-block-editor',
        'render_callback' => 'cvt_render_block',
    ));
}
add_action('init', 'cvt_register_block');

// Render callback for the block
function cvt_render_block($attributes, $content) {
    return do_shortcode('[christine_valmy_test]');
}
...
// Add hook for theme integration
function cvt_add_to_homepage() {
    if (is_front_page()) {
        echo do_shortcode('[christine_valmy_test]');
    }
}
// Uncomment the line below to automatically add to homepage
// add_action('wp_head', 'cvt_add_to_homepage');

// Register widget
class CVT_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
            'cvt_widget',
            'Christine Valmy Chat',
            array('description' => 'Interactive chat interface for Christine Valmy')
        );
    }
    ...
    public function form($instance) {
        echo '<p>This widget displays the Christine Valmy chat interface.</p>';
    }
}

function cvt_register_widget() {
    register_widget('CVT_Widget');
}
add_action('widgets_init', 'cvt_register_widget');
